...
For example don't merge from `main` to `documentation`.

* `main` (protected) - default branch.
Direct pushing not allowed.
Merge requests (except software merge requests) should be created against this branch.
* `documentation` - for editing main README.asciidoc and any other general documentation in root directory.
* `software` (protected) - for robot's code and related documentation in `software` directory.
Software merge requests should be created against this branch.
...
Example for electronics people:
[source,bash]
----
git clone --single-branch --branch documentation git@gitlab.ut.ee:/loti/loti.05.023/picr2025/picr25-team-<team-name>.git picr25-team-<team-name>-documentation
...
* Feature branches should be created from `software` branch.
** See: xref:basketball-robot-guide/software/useful-links.adoc#feature_branches[Feature branches guide]
* When the functionality for a programming task has been implemented and thoroughly tested on the court,
a merge request from feature branch to `software` should be created and instructors notified about it.
