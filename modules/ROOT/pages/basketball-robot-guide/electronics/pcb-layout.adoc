...
. Route the signals and power, and connect ground with vias to internal layers.
Adjust component placement where necessary to make room for vias and tracks or to bring components closer to each other.
. Create ground copper areas on internal layers.
...
** When components are hand soldered, then it's fine to place the vias inside the capacitor's pads.
** When reflow soldering, vias would need to be plugged/filled first to avoid solder paste flowing into the via holes.

image::pcb_layout_decoupling_capacitor.png[Decoupling capacitor connection]
...
* link:https://www.analog.com/en/resources/technical-articles/successful-pcb-grounding-with-mixedsignal-chips--follow-the-path-of-least-impedance.html[]
...
image::pcb_layout_vias_gap.png[Gap from vias]

=== Thermal pads

Connect thermal pads to the ground copper areas with multiple vias.

image::pcb_layout_thermal_pad_vias.png[Thermal pad vias]