...
image::schematic_net_labels.png[Net labels,600]

=== Supply connections

Avoid connecting all the power supply pins together. Use power supply symbols instead.

image::schematic_supply_symbols.png[Supply symbols,600]
...
image::schematic_decoupling_capacitors.png[Net labels,600]

== Programmer connector

One place where to find the pinout for programmer connector is in
link:https://www.st.com/resource/en/user_manual/um2910-stlinkv3minie-debuggerprogrammer-tiny-probe-for-stm32-microcontrollers-stmicroelectronics.pdf[STLINK-V3MINIE user manual]
under _6.3 STDC14 connector (CN4)_.

On the schematic the connections should look similar to the image below:

image::schematic_programmer_connector_arm10.png[Programmer connector,400]
...
If nSWBOOT0 is set to 0, then pulldown resistor should be connected to BOOT0 pin.
For more information see Boot configuration section on
xref:basketball-robot-guide/electronics/firmware.adoc[Firmware] page.
