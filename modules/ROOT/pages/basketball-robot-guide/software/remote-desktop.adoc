...
You can download the rpm package from https://www.nomachine.com/ and install it using your preferred installer.
One way would be:
+
[source,bash]
----
...
. Remember that the HDMI/DP won't output anything anymore. If you need the outputs again, just rename or remove the
created *xorg.conf* file. Do it over SSH or if there are network issues over the recovery terminal.